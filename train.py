...
SAVE_EVERY = 5000 #
NUM_EPISODES = 150      # Start small for debugging
...
def optimize_model(model, memory, optimizer, batch_size, device, gamma=0.99):
    ...
    states      = torch.tensor(np.array(states), dtype=torch.float32, device=device).view(batch_size, -1)
    actions     = torch.tensor(actions, dtype=torch.int64, device=device).unsqueeze(1)
    rewards     = torch.tensor(rewards, dtype=torch.float32, device=device).unsqueeze(1)
    next_states = torch.tensor(np.array(next_states), dtype=torch.float32, device=device).view(batch_size, -1)
